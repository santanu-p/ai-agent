...
def test_service_workflow_end_to_end_and_learning(tmp_path: Path) -> None:
    ...
    assert response["goal_status"] == "completed"
...
def test_scheduler_queue_and_run(tmp_path: Path) -> None:
    service = AegisWorldService(state_file=str(tmp_path / "state.json"))
    agent = service.create_agent({"name": "queue-agent"})
    goal = service.create_goal({"intent": "queued workflow", "domains": ["dev"]})

    queued = service.assign_goal({"agent_id": agent["agent_id"], "goal_id": goal["goal_id"]})
    assert queued["queued"] is True
    assert queued["queue_length"] == 1

    result = service.run_scheduler(max_runs=1)
    assert len(result["executed"]) == 1
    assert result["remaining_queue"] == 0
    updated_goal = service.get_goal(goal["goal_id"])
    assert updated_goal is not None
    assert updated_goal["status"] in {"completed", "blocked"}


def test_policy_update_can_cause_blocked_execution(tmp_path: Path) -> None:
    service = AegisWorldService(state_file=str(tmp_path / "state.json"))
    agent = service.create_agent({"name": "strict-agent"})
    goal = service.create_goal({"intent": "strict workflow", "domains": ["dev"]})

    service.update_agent_policy(
        agent["agent_id"],
        {
            "tool_allowances": ["planner"],
